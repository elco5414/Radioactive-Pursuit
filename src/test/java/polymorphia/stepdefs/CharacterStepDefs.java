...
public class CharacterStepDefs {
    ...
    public void characterPickedUpTheArmor(String characterName) {
        ...
        Character armoredCharacter = currentRoom.getLivingCharacters().stream()
                ...
                .findFirst()
                .get();
        assertTrue(armoredCharacter.getName().contains("Armor"));
    }

    @Then("{word} picked up the Weapon")
    public void characterPickedUpTheWeapon(String characterName) {
        Character character = world.characters.get(characterName);
        Room currentRoom = character.getCurrentLocation();
        Character armoredCharacter = currentRoom.getLivingCharacters().stream()
                .filter(c -> c.getName().contains(characterName))
                .findFirst()
                .get();
        assertTrue(armoredCharacter.getName().contains("Weapon"));
    }
    ...
    @And("{word} has lost the default fight cost in health")
    public void characterHasLostTheDefaultFightCostInHealth(String characterName) {
        Character character = world.getCharacter(characterName);
        Double initialHealth = world.getInitialHealth(characterName);
        assertEquals(initialHealth - Character.HEALTH_LOST_IN_FIGHT_REGARDLESS_OF_OUTCOME, character.getHealth());
    }
    ...
    public void galahadLostTheDefaultMovingCostPlusInHealth(String characterName, double extraHealthLost) {
        Double DEFAULT_MOVING_HEALTH_LOST = 0.25;
        Character character = world.getCharacter(characterName);
        Double initialHealth = world.getInitialHealth(characterName);

        Double amountIncurredMovingRooms = DEFAULT_MOVING_HEALTH_LOST + extraHealthLost;
        Double charFinalHealth = initialHealth - amountIncurredMovingRooms;

        assertEquals(charFinalHealth, character.getHealth());}

    @And("{word} has lost {int} times the default fight cost in health")
    public void hasLostTimesTheDefaultFightCostInHealth(String characterName, int timesFought) {
        Character character = world.getCharacter(characterName);
        Double initialHealth = world.getInitialHealth(characterName);

        //this test does not account for the amount of health lost during the actual fight, just that AT LEAST the default has decremented
        Double amountOfHealthLostAsDefault = Character.HEALTH_LOST_IN_FIGHT_REGARDLESS_OF_OUTCOME * timesFought;
        Double characterHealthLostTotal = initialHealth - character.getHealth();

        assertTrue(characterHealthLostTotal >= amountOfHealthLostAsDefault);
    }

    @And("{word} has lost {double} points in health")
    public void monicaHasLostPointsInHealth(String characterName, double healthLost) {
        Character character = world.getCharacter(characterName);
        Double initialHealth = world.getInitialHealth(characterName);

        assertEquals(healthLost, initialHealth - character.getHealth());
    }
}
